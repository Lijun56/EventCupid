  ...
  const fetchVendors = async () => {
    ...
    try {
      ...
      const response = await fetch(
        ...
        }/vendors/search?${params.toString()}`
      );
          ...
          <ul style={{ listStyle: "none", padding: 0 }}>
            {vendors.map((vendor, index) => (
              <li key={index} style={vendorItemStyle}>
                <strong style={vendorNameStyle}>{vendor.name}</strong> -{" "}
                <span style={vendorCategoryStyle}>
                  {vendor.service_category}
                </span>
                <p>
                  <em>Description:</em> {vendor.description}
                </p>
                <p>
                  <em>Base Price:</em> ${vendor.base_price}
                </p>
              </li>
            ))}
          </ul>
