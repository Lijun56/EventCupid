  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/events/add`,
        {
          method: "POST",
          headers: {
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ marginBottom: "20px" }}>
          <label
            htmlFor="eventName"
            style={{ display: "block", color: "#D1D1D1" }}
          >
            Event Name:
          </label>
          <input
      ...
      {error && (
        <p style={{ color: "#FF4C4C", textAlign: "center", marginTop: "10px" }}>
          {error}
        </p>
      )}
      {success && (
        <p style={{ color: "#28A745", textAlign: "center", marginTop: "10px" }}>
          {success}
        </p>
      )}
    </div>
